...
def test_sigmoid(a: float) -> None:
    s = sigmoid(a)
    assert 0.0 <= s <= 1.0  # Always between 0 and 1
    assert_close(s, 1.0 - sigmoid(-a))  # Symmetric property
    assert_close(sigmoid(0.0), 0.5)  # Sigmoid(0) should be 0.5
...
def test_symmetric(a: float, b: float) -> None:
    assert_close(mul(a, b), mul(b, a))  # a * b should be the same as b * a
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    assert_close(left, right)  # z * (x + y) == z * x + z * y


@pytest.mark.task0_2
@given(small_floats)
def test_other(a: float) -> None:
    assert_close(neg(neg(a)), a)  # Double negation should return original value
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    """Ensure that sum(ls1) + sum(ls2) == sum(addLists(ls1, ls2))."""
